...
class PointFoot:
    ...
    def _post_physics_step_callback(self):
        ...
        self._resample(env_ids)
        self._update_reference_phase(self.dt)
    ...
    def _calc_targets(self, dt):
        self._update_reference_phase(dt)
        refs = self.references.get_references(self.reference_idxs, self.phases)
    ...
    def update_command_curriculum(self, env_ids):
        ...
        if torch.mean(self.episode_sums["tracking_lin_vel"][env_ids]) / self.max_episode_length > 0.8 * \
            ...
            self.command_ranges["lin_vel_y"][0] = np.clip(self.command_ranges["lin_vel_y"][0] - 0.5,
                                                          -self.cfg.commands.max_curriculum, 0.)
            self.command_ranges["lin_vel_y"][1] = np.clip(self.command_ranges["lin_vel_y"][1] + 0.5, 0.,
                                                          self.cfg.commands.max_curriculum)
            self.command_ranges["ang_vel_z"][0] = np.clip(self.command_ranges["lin_vel_z"][0] - 0.5,
                                                          -self.cfg.commands.max_curriculum, 0.)
            self.command_ranges["ang_vel_z"][1] = np.clip(self.command_ranges["lin_vel_z"][1] + 0.5, 0.,
                                                          self.cfg.commands.max_curriculum)
    ...
    def _reward_salsa_foot_contact(self):
        err = torch.abs(self.contact_filt.float() - self.target_foot_contacts.float())
        # Reward is negative error
        return -torch.sum(err, dim=1)

    def _reward_salsa_foot_xy(self):
        foot_xy = self.foot_positions[:, :, :2]
        target_xy = self.target_foot_xy_rel_chassis
        xy_err = torch.norm(foot_xy - target_xy, dim=2)
        # Reward is negative error
    ...
    def _reward_feet_regulation(self):
        feet_height = self.cfg.salsa.foot_height
        reward = torch.sum(
